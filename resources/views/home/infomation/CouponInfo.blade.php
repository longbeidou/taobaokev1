...
<html lang="zh-CN">
  <head>
    ...
    <title>{{ $info->goods_name }} - {{config('website.name')}}</title>
    ...
    // window.setInterval(function(){ShowCountDown(2017,8,14,'couponendtime');}, interval); 
    window.setInterval(function(){ShowCountDown({{$info->date[0]}},{{$info->date[1]}},{{$info->date[2]}},'couponendtime');}, interval); 
    </script> 
  ...
  <body>
    <div class="container-fluid">
        ...
        <div class="row bgwhite paddingtop">
            <strong class="coupontitle">{{ $info->goods_name }}</strong>
        </div>
        <div class="row bgwhite">
            <span class="qhby">券后包邮</span>
            <span class="pricenow">{{ $info->price_now }}</span>
        ...
        <div class="row bgwhite">
            <table class="table table-condensed " style="margin-bottom:0px;">
                <tbody>
                    <tr>
                        <td>券后价:{{ $info->price_now }}</td>
                        <td>在售价:<del>{{ $info->price }}</del></td>
                        <td>优惠占比:{{ $info->rate_sales*100 }}%</td>
                    </tr>
                    <tr>
                        <td>月销量:{{ $info->sales }}</td>
                        <td>来源: 
                            @if($info->flat == '天猫')
...
<!--         <div class="row bgwhite margintop">
        ...
        <div class="row bgwhite paddingtop paddingbottom">
            ...
            <div class="col-xs-6">
                <a href="{{ route('urlCouponInfo',['id'=>$goods->id]) }}">
                    <img src="{{ $goods->image }}" width="100%" @if($key == 0) id="picBox" @endif class="picBox">
                </a>
                <span class="coupontitlemin">
                    <a href="{{ route('urlCouponInfo',['id'=>$goods->id]) }}">{{ $goods->goods_name }}</a>
                </span>
                <div class="row">
                    <span class="pricenowmin">￥{{ $goods->price_now }}</span>
                    <del class="pricemin">￥{{ $goods->price }}</del>
                </div>
                <div class="row">
                    <div class="col-xs-7 text-center couponyouhuimin text-nowrap">{{ $goods->yhq_info }}</div>
                    <div class="col-xs-5 text-center coupontakeaway text-nowrap">
                        <a href="{{ route('urlCouponInfo',['id'=>$goods->id]) }}">马上领券</a>
                    </div>
                </div>
            </div>
                @if($key%2 == 1)
              ...
              <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="methodtwo" style="font-size:10px;">
                        ...
                        <div class="twocode text-center" style=" font-size:14px; color:red;">淘口令：{{ $info->tao_kou_ling }}</div>
                        <div class="text-center" style="margin-bottom:10px; margin-top:10px;">
                            <!-- <button class="btn btn-danger btn-sm" style="border-radius: 50px; ">一键复制淘口令</button> -->
                        </div>
